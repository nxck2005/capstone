...
**Last updated:** 2026-09-10 · **Phase:** **W9 ER-9/ER-2/G-11 pre-science hardening; PASCAL EXEC READY is held for the Confessor outage.** AM-97 is frozen as a pointwise H4 precision diagnostic only. The v3 local ER-9 runtime has been authenticated and preserved as scientifically ineligible local-profile history; it has no accepted real-chain evaluation or Stage-1 selection. W9 v4 transactional recovery, source closure, outage scoring, ER-2 resume/AMP accounting, SR-18 rows, H4 diagnostic code and downstream historical compatibility are implemented and locally gated. No Pascal smoke, exact GPU freeze, v4 source manifest, Stage-1 authority or prelaunch success claim is issued while Confessor is unavailable. ER-9 v4, randomized ER-2, G-11, W10, learned test inference and model-facing test access remain zero; **test is SEALED**.

**W9 recovery hold (2026-09-10):** Confessor is currently unreachable because of a power outage. Do not substitute the local RTX4060, choose a Pascal GPU without live evidence, run the synthetic Pascal smoke, freeze v4 source, create the exact Stage-1 authority, or launch science. Resume those read-only/live-host steps only after Confessor and the exact qualified GPU can be authenticated.
...
```bash
.venv/bin/python tools/gen_spec_views.py --check           # expect: 207 requirements (2 retired)
