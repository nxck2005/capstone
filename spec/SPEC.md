...
- ~~§2's rewrite needs **supervisor ratification**.~~ **Settled 2026-07-25**: the supervisor's position is to pursue a crossover as hard as the rules allow and to fall back to reporting dominance across the grid if it does not appear — which is DEC-16's existing structure, now ratified externally (AM-19). The proposal's "curves must cross" wording is therefore superseded rather than silently redefined, which was the whole reason this item existed. **Residual, and narrower:** confirm the ratification covered §2 *as a whole* — completion-not-outcome, the four hypotheses, and paired inference — and not only the crossover clause. If the conversation was about the crossover alone, the rest of §2 still needs an explicit sign-off before G-9 closes, and it is a much easier conversation now that the headline question is settled.
