...
invoke fallback, adjudicate ratios or access test. **AM-87 resolves the
...
global quality counts 423–424, every class range 0–1, no duplicate pair. The
owner independently accepted AM-88 and authorized **F0 only** on 2026-08-24.
The first F0 remains byte-identical historical evidence at
...
and is explicitly superseded-before-F1 because its resume scan did not fully
authenticate result bodies or referenced object bytes. **F0-v2 is GREEN and
current** at `results/baseline/g8_f/f0_v2_execution_authorization.json`
...
binding repaired F1 source commit `b1ee63d95de4fe86b9758ae90dbbb7b428a63635`, profile
`local_4060_cu130`, lock SHA `ee68e232…`, and the exact AM-88 assignment.
No production artifact, real F1 JPEG2000 invocation, classifier inference,
optimizer step, pass two or test access occurred; the F1 runtime and F1 launch
authorization are absent. **F1 is not started and requires a separate
owner/operator launch action.**
