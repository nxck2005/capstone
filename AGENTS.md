...
**Implementation started 2026-07-28. W1, W2 and W3 are complete; G-9, G-1, G-7 and G-2 passed; the validation-only transparency-bitrate probe is complete and lineage-bound.** W4 and G8_A are complete. **G8_B is complete through B6:** the authenticated runner, exact resume/merge tooling, atomic state publication and verified bounded smoke are green; `G8_B/tooling_smoke_complete` is the live cursor. G8_C is ready but not started. No full-strength characterization, BLER table, selection, authorization, inference, training, validation decoding or test access has occurred. The test split remains sealed, and G8_C–G8_G remain prohibited until explicitly opened by the existing phase protocol. The committed G-2 BLER evidence remains a narrow conformance artifact and must not be extrapolated into the BR-4 characterization table. Do not calibrate λ, train learned models, implement ER-9 or evaluate on test before their gates.

**Current action: G8_B complete; G8_C is ready but not started.** On resumption, `instructions/RESUME.md` remains the single operational cursor. The exact G8_C restart command stored in campaign state is `rg -n "G8_C|characterization_open|full_strength|run_g8_bler|resume_plan|merge_report|tooling_smoke_complete" src/baseline tools tests instructions`. Do not enter G8_C, create authorization or execute full-strength work in this handoff.

**Final G8_B handoff:** the live cursor is `G8_B/tooling_smoke_complete` with campaign-state
