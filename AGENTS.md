...
**Implementation started 2026-07-28. W1, W2 and W3 are complete; G-9, G-1, G-7 and G-2 passed; the validation-only transparency-bitrate probe is complete and lineage-bound.** The frozen G-1 checkpoint remains `9c37362347a0203597d6e8e9d9a58fde30ba286f3cec9b4d2f800bd8a3256002` with config `a9717575d71f2b3e9dd411b10b7735bdb3946c985fead48cb3c5af07423f12e1`; W2's immutable implementation commit remains `26b631ede27a6f88f1d004a66b845c52a658e07c`. The probe reproduced 898/1000 over 68,000 validation cells with zero infeasible encodes or decode failures and did not select a G-8 ratio. W3's immutable implementation commit is `968e907237bbe571adf6ec48e4711ea021831719`. G-2 passed its srsRAN exact vectors, project-owned offline floor, mapper/demapper/interleaver and CRC known answers, independent flooding-OMS BLER comparison, and all 216 packetisation/runtime cells: waterfall displacements were 0.0 dB for BPSK, +0.0036302379 dB for QPSK and 0.0 dB for 16-QAM against the 0.5 dB limit. The test split stayed sealed and no model training ran. The single next task is **bounded W4 classical-baseline integration required before G-8**. Do not begin the full BR-4 validation sweep, G-8 selection, λ calibration, learned training, ER-9 or test evaluation.

**W4 progress: PA and PB_1 (including its PB_1C correction) are complete. PB_2's implementation landed but its bounded evidence and green judgment are superseded pending the PB_2C corrective repair, which is the current phase; PB_3 is blocked by PB_2C.** PB_2C resolves two questions PB_2 left open — **AM-80** cuts `params.baseline.downsample_axis_px.cifar10` to the single native `[32]` rung, because a flat `j2k_resolutions` of 6 needs every tile dimension ≥ 32 px and OpenJPEG rejects 24 px and 16 px before rate control; **AM-81** defines the BR-11 `header_bytes`/`payload_bytes` columns arithmetically and aggregates them over every emitted codestream (delivered *and* decode-failure) rather than delivered rows only, bumping `params.config.analysis_version` to 2. AM-80 moves the content-addressed codec configuration hash and therefore **every J2K cache key**; **AM-82** binds the completed transparency-bitrate probe's codec configuration as history and permits that one difference only through a single byte-pinned off-measurement-path record — not an allowlist, and the probe was not re-run. `instructions/PB_2C.txt` is the durable instruction (its §18 addendum is binding) and `instructions/RESUME.md` is the operational cursor. PB_1C removed a duplicated TS 38.212 §5.4.2.2 modulation bit interleaver from the classical transmit path: `SionnaLDPCAdapter` builds `LDPC5GEncoder` with `num_bits_per_symbol=q_m`, so Sionna applies that permutation after rate matching (`encoding.py:791-793`) and its inverse before rate recovery (`decoding.py:1646-1649`) — and `channel_transport.modulate()` was applying the *same* permutation a second time. PB_1's round-trip tests could not see it, because the paired transmit and receive errors cancel and bit counts are permutation-invariant; `tests/test_classical_interleaver_conformance.py` now checks the bit order entering the mapper against an independently derived reference. The repair touched **no file under `src/baseline/ldpc/`**, so G-2 remains valid through the existing exact-byte `transport.py` re-adjudication and no new one was added. G-2 runtime drift otherwise fails closed: a changed runtime file raises a HOLD unless covered by a verified byte-pinned readjudication. `instructions/RESUME.md` is the operational cursor; `instructions/PB_1C.txt` is the durable instruction copy.
...
`results/baseline/w4/execution_source_manifest.json` binds 37 sources at the clean runner-ready
...
.venv/bin/python tools/verify_w4_baseline_integration.py # network-free bounded W4 evidence cross-check
