...
**Where the spec stands.** It now carries 122 requirements (1 retired), of which 25 are `AM` amendment records. **W0 is done and gate G-9 has passed (2026-07-27), so W1 is open** — the LDPC spike ran clean on the primary device and its findings are AM-24 (the measured half, plus three defects it caught: an LLR sign convention that fails *silently*, a nominal code rate that was unrealizable at three live operating points, and a decoder-name mismatch) and AM-25 (golden vectors — Sionna matches srsRAN's MATLAB-generated vectors bit-exactly, and the AGPL question dissolved once it emerged that the vector data was never committed upstream). Still true that **no implementation code exists**: W0 produced measurements and spec changes, not project code. A fifth external review (2026-07-25, ChatGPT) was adjudicated rather than deferred to: roughly a third was adopted, its central objection to H1's run rule was rejected on the arithmetic in AM-4, three of its recommendations restated decisions already settled in DEC-10/DEC-14/G-9, and the internal audit run alongside it found five further defects — including AM-14, where ER-3 required more validation images than any dataset has. **Read §17 first if you are wondering why something is the way it is.** Before that, it was substantially revised (97 requirements, 1 retired) after four independent reviews converged on five blockers, all now addressed: §2's crossover requirement was unachievable at the specified operating point; JPEG's container floor made the classical baseline degenerate; baseline tuning leaked the test split; the statistical design could not support its own claims; and ~30 rubric marks of graded deliverables were unscheduled. **`SPEC.md` §16 is the open-items and risk register** — read it first when resuming. It lists what still needs supervisor sign-off, which `params` values are provisional estimates awaiting a gate (`core_ratio`, `lambda_core`, the `clean_acc_floor`s), and the risks knowingly carried. W0 has now been executed and **G-9 has passed**; W1 is the current front of the queue.
