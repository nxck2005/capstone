...
**Last durable checkpoint:** `40f115e67f0bfd820c59eb0f6393cab3157265f3` (C1.1)
**Next action:** remove the project-side `interleave()` from `mapper_input_bits()` and the `deinterleave()` from `demodulate()` in `src/baseline/classical/channel_transport.py` (C1.3)
