...
def build_scenes() -> list[SlideScene]:
    ...
    s.text(0.72, 1.48, 11.86, 0.45,
           "Can a neural model learn what an edge AI task needs to communicate—without sending every pixel perfectly?",
    ...
    s.text(1.33, 5.03, 10.66, 0.42,
           "Train a residual neural encoder and dual-head decoder end to end through a differentiable noisy channel; classification provides the measurable AI task.",
    ...
    s.text(1.99, 6.32, 9.35, 0.19,
           "AI/ML goal: learn a compact, channel-robust representation that preserves downstream inference accuracy.",
           size=11.2, font="serif", bold=True, text_color=AMBER, align="center")
    add_footer(s); slides.append(s)

    # 3 — intuitive idea before the formal comparison
    s = SlideScene(3, "Core idea", "From bit-perfect delivery to learned AI representations",
                   ("Motivation", "Objectives"),
    ...
    lanes = [
        ...
        (3.38, "NEURAL TASK-ORIENTED LINK", "Residual encoder → differentiable noise → dual-head decoder → class logits", BURGUNDY, PALE_RED),
    ]
    for y, heading, body, accent, pale in lanes:
        ...
        s.text(3.48, y + 0.78, 8.42, 0.27,
                "Primary objective: " + ("recover source bits and pixels" if accent == NAVY else "learn a channel-robust representation for inference"),
    ...
    s = SlideScene(4, "Problem survey", "How AI/ML and communication research lead here",
    ...
    # 6 — architecture
    s = SlideScene(6, "Architecture", "AI/ML architecture and three-arm experiment",
    ...
    # 8 — evidence
    s = SlideScene(8, "Evidence", "AI/ML feasibility and system evidence",
                   ("Subject Knowledge",),
    ...
    gantt_rows = [
        ("BLER characterization + G-8", date(2026, 8, 9), date(2026, 8, 23), "PAUSED → PLANNED", NAVY, WHITE),
        ("First Review package", date(2026, 8, 11), date(2026, 8, 23), "ACTIVE / REVIEW", BURGUNDY, WHITE),
        ("Neural training + λ calibration", date(2026, 8, 24), date(2026, 9, 21), "PLANNED", GREEN, WHITE),
        ("AI feature control + validation", date(2026, 9, 21), date(2026, 10, 5), "PLANNED", BURGUNDY, WHITE),
