...
def _build_pre_knowledge_transfer_scenes() -> list[SlideScene]:
    ...
    sections = [
        ...
        ("Decision requested", "Confirm the task boundary, the three-system comparison, the fairness controls, and simulation-first Tier 1 as a sufficient completion path."),
    ]
...
def build_minimal_scenes() -> list[SlideScene]:
    ...
    s = SlideScene(1, "Title", "Task-Oriented Deep Joint Source–Channel Coding",
                   ("Motivation", "Objectives"),
                   "First Review · 18–22 August 2026")
    s.text(0.74, 1.00, 11.80, 1.20,
           "Task-Oriented Deep Joint Source–Channel Coding",
           size=34, bold=True, font="sans", text_color=INK)
    s.text(0.76, 2.42, 11.35, 0.50,
           "Semantic communication for bandwidth-constrained image classification",
           size=19, font="sans", text_color=MUTED)
    s.line(0.76, 3.18, 11.78, 0, stroke=LINE, stroke_width=0.9)
    s.text(0.76, 3.62, 11.30, 0.84,
           "The project studies whether an edge device can send the information needed for a classification task without having to transmit and reconstruct every image detail.",
           size=17, font="sans", text_color=MUTED)
    ...
    s = SlideScene(2, "Overview", "The project in one minute",
                   ("Motivation", "Objectives"),
                   "Tier 1 is a simulation-first image-classification experiment")
    minimal_header(s, s.title)
    steps = [
        ("1", "Observe", "An edge camera sees an image."),
        ("2", "Send", "The link has limited bandwidth and added noise."),
        ("3", "Decide", "A receiver predicts the image class."),
        ("4", "Compare", "We measure task accuracy at the same channel budget."),
    ]
    ...
    s.text(1.02, 4.68, 3.15, 0.30, "Semantic communication", size=16,
           bold=True, font="sans", text_color=INK)
    s.text(4.18, 4.64, 7.98, 0.56,
           "Send the information required to complete the task. Reconstructing every original pixel is optional when the task does not need it.",
           size=15, font="sans", text_color=MUTED)
    s.text(0.78, 6.14, 11.55, 0.48,
           "The final result is classification accuracy after transmission, not image quality by itself.",
           size=15, bold=True, font="sans", text_color=INK, align="center")
    minimal_footer(s); slides.append(s)

    # 3 — ordinary system, with each term introduced in context
    s = SlideScene(3, "Conventional approach", "How a conventional image link solves the task",
    ...
    path = [
        ...
        (9.72, 1.62, 2.90, "Recover and classify", "image → class label"),
    ]
    ...
    definitions = [
        ...
        ("Classifier", "uses the recovered image to choose a class label."),
    ]
    ...
    s.text(0.80, 6.20, 11.42, 0.44,
           "Limitation: each stage is designed mainly to recover images and bits, while the final metric is whether the class decision is correct.",
           size=14.2, bold=True, font="sans", text_color=INK)
    minimal_footer(s); slides.append(s)

    # 4 — learned change, including the class-label objection
    s = SlideScene(4, "Proposed approach", "What the learned joint system changes",
                   ("Motivation", "Objectives", "Subject Knowledge"),
                   "DJSCC = deep joint source–channel coding")
    minimal_header(s, s.title)
    learned_path = [
        (0.86, 1.62, 1.72, "Image", "sender input"),
        (3.10, 1.62, 2.14, "Neural encoder", "learned representation"),
        (5.76, 1.62, 1.86, "Noisy channel", "used during training"),
        (8.14, 1.62, 2.30, "Neural decoder", "recover task information"),
        (10.96, 1.62, 1.46, "Class", "receiver output"),
    ]
    ...
    for x in (2.68, 5.34, 7.72, 10.54):
        s.arrow(x, 2.28, 0.28, 0, stroke=MUTED, stroke_width=1.0)
    s.text(0.82, 3.48, 3.08, 0.34, "The central change", size=17,
           bold=True, font="sans", text_color=INK)
    s.text(3.92, 3.43, 8.22, 0.70,
           "The sender and receiver are trained together through the channel. The transmitted representation can therefore preserve information useful for classification after noise.",
           size=15, font="sans", text_color=MUTED)
    ...
    s.text(1.02, 6.04, 10.98, 0.30,
           "The method is supervised end-to-end learning. It is not reinforcement learning and it does not use an LLM.",
           size=13.5, bold=True, font="sans", text_color=INK, align="center")
    minimal_footer(s); slides.append(s)

    # 5 — why three systems are scientifically necessary
    s = SlideScene(5, "Research design", "Why the comparison needs three systems",
                   ("Objectives", "Subject Knowledge"),
                   "The third system separates task awareness from joint coding")
    minimal_header(s, s.title)
    widths = (2.55, 4.45, 4.70)
    minimal_table_row(s, 1.48, widths, ("System", "What it sends", "What the comparison tells us"), header=True,
                      sizes=(10.5, 10.5, 10.5))
    rows = [
        ("Classical image link", "A compressed image protected by LDPC and modulation.", "How a strong conventional separated system performs."),
        ("Digital feature control", "Quantized learned features through the same digital physical layer.", "Whether task-aware representation alone explains an improvement."),
        ("Learned joint link", "A continuous representation learned together with the noisy channel.", "Whether joint source–channel learning adds further value."),
    ]
    for i, row in enumerate(rows):
        minimal_table_row(s, 2.10 + i * 1.05, widths, row, h=1.05,
                          sizes=(12.3, 12.0, 12.0))
    s.rect(0.72, 5.52, 11.70, 1.08, fill="F5F5F5", stroke=None, radius=0)
    s.text(0.96, 5.74, 2.18, 0.26, "Research question", size=13,
           bold=True, font="sans", text_color=INK)
    s.text(3.06, 5.69, 9.02, 0.62,
           "At the same bandwidth and channel conditions, can the learned joint system preserve classification accuracy better than the tuned classical system, and is any difference specifically consistent with joint coding?",
           size=13.5, font="sans", text_color=INK)
    minimal_footer(s); slides.append(s)

    # 6 — prior knowledge first, technical gap second
    s = SlideScene(6, "Problem survey", "What prior research establishes—and leaves open",
                   ("Problem Survey", "Subject Knowledge"),
                   "30-source synthesis: docs/literature-review.md")
    minimal_header(s, s.title)
    topics = [
        ...
        ("Learned image compression", "Known", "Neural transforms can improve rate–distortion performance.", "Good reconstruction quality is not the same as good task accuracy. [11–17]"),
        ("DeepJSCC", "Known", "Learned continuous mappings can degrade gradually under noise.", "Most image studies emphasize reconstruction rather than classification. [5–10,24]"),
        ("Task-oriented communication", "Known", "Learned features can preserve inference under link limits.", "Task awareness and joint coding are often not isolated from each other. [18–23]"),
    ]
    ...
    s.text(3.42, 6.12, 8.78, 0.50,
           "A fair three-way comparison for image classification at equal channel use, with failures retained and one sealed test campaign.",
           size=13.4, font="sans", text_color=MUTED)
    minimal_footer(s); slides.append(s)

    # 7 — one mental model for the whole experimental protocol
    s = SlideScene(7, "Methodology", "How one image becomes one fair comparison",
                   ("Subject Knowledge", "Objectives"),
                   "SPEC BR-4, BR-10/11 and ER-3/9/10")
    minimal_header(s, s.title)
    s.text(0.78, 1.48, 2.70, 0.32, "Held equal for all systems", size=16,
           bold=True, font="sans", text_color=INK)
    s.text(6.86, 1.48, 2.90, 0.32, "Why the control exists", size=16,
           bold=True, font="sans", text_color=INK)
    controls = [
        ("Same image and dataset split", "Prevents easier samples from being assigned to one system."),
        ("Same complex-symbol budget", "Makes bandwidth use directly comparable."),
        ("Same SNR definition and keyed noise", "Pairs outcomes under the same channel condition."),
        ("Validation-only tuning", "Keeps all choices away from the sealed test split."),
        ("Every failure stays in the denominator", "Prevents failed transmissions from disappearing from accuracy."),
        ("Full byte and physical-layer identity", "Prevents hidden overhead or missing BLER evidence from helping a system."),
    ]
    for i, (control, reason) in enumerate(controls):
        y = 1.98 + i * 0.70
        s.text(0.80, y, 5.52, 0.34, control, size=13.7, bold=True,
               font="sans", text_color=INK)
        s.text(6.88, y - 0.02, 5.30, 0.40, reason, size=13.2,
               font="sans", text_color=MUTED)
        s.line(0.78, y + 0.50, 11.42, 0, stroke=LINE, stroke_width=0.45)
    s.rect(0.78, 6.28, 11.42, 0.42, fill="F5F5F5", stroke=None, radius=0)
    s.text(0.96, 6.38, 11.06, 0.18,
    ...
    # 8 — objectives and hypotheses after the audience understands the method
    s = SlideScene(8, "Objectives and hypotheses", "What the project will complete and test",
                   ("Objectives", "Hypothesis"),
                   "Formal H1 decision rule appears on slide 12 · exact protocol: SPEC §2")
    minimal_header(s, s.title)
    s.text(0.78, 1.46, 5.28, 0.34, "Completion objectives", size=17,
           bold=True, font="sans", text_color=INK)
    objectives = [
        "Build all three transmission systems.",
        "Match their communication resources and channel conditions.",
        "Freeze every choice before test access.",
        "Run one paired test campaign and report the result either way.",
    ]
    for i, item in enumerate(objectives, 1):
        y = 1.98 + (i - 1) * 0.70
        s.text(0.84, y, 0.38, 0.28, f"{i}.", size=13.5, font="sans", text_color=MUTED)
        s.text(1.28, y - 0.02, 4.90, 0.46, item, size=13.6,
               font="sans", text_color=MUTED)
    s.line(6.52, 1.44, 0, 4.76, stroke=LINE, stroke_width=0.7)
    s.text(6.84, 1.46, 5.28, 0.34, "Preregistered questions", size=17,
           bold=True, font="sans", text_color=INK)
    hypotheses = [
        ("H1", "At low SNR, does the learned system outperform the adaptive classical system?"),
        ("H2", "Does a fixed classical configuration show a cliff while the learned system degrades more gradually?"),
        ("H3", "Does the learned–classical gap contract as SNR improves?"),
        ("H4", "Does the learned joint system also exceed the digital feature control?"),
    ]
    for i, (tag, question) in enumerate(hypotheses):
        y = 1.98 + i * 0.86
        s.text(6.86, y, 0.56, 0.28, tag, size=13.5, bold=True,
               font="sans", text_color=INK)
        s.text(7.48, y - 0.02, 4.66, 0.58, question, size=12.7,
               font="sans", text_color=MUTED)
    s.rect(0.78, 5.88, 11.42, 0.70, fill="F5F5F5", stroke=None, radius=0)
    s.text(1.00, 6.08, 10.98, 0.30,
           "Completion means executing the protocol correctly. A positive learned-system result is not required.",
           size=13.5, bold=True, font="sans", text_color=INK, align="center")
    minimal_footer(s); slides.append(s)

    # 9 — status in plain meaning, with no volatile campaign counts
    s = SlideScene(9, "Current status", "What has been demonstrated so far",
                   ("Subject Knowledge",),
                   "Engineering and gate evidence only · no learned-versus-classical result")
    minimal_header(s, s.title)
    widths = (1.18, 3.62, 3.42, 3.48)
    minimal_table_row(s, 1.46, widths, ("Gate", "Observed evidence", "Plain-language meaning", "Boundary"),
                      header=True, sizes=(10.0, 10.0, 10.0, 10.0))
    rows = [
        ("G-1", "89.8% validation accuracy", "The reference image-classification task is viable.", "Validation only; test is sealed."),
        ("G-7", "1.64 M parameters; measured GPU profile", "The planned learned model fits the available GPU.", "A feasibility profile, not training."),
    ...
    # 10 — honest remaining path and hard dates
    s = SlideScene(10, "Time plan", "What remains after First Review",
                   ("Time Plan",),
                   "docs/gantt-plan.md · unfinished stages remain marked unfinished")
    minimal_header(s, s.title)
    widths = (2.02, 2.20, 4.82, 2.66)
    minimal_table_row(s, 1.44, widths, ("Stage", "State", "Work", "Output"), header=True,
                      sizes=(10.2, 10.2, 10.2, 10.2))
    rows = [
        ("G8_C → G-8", "Paused → planned", "Finish BLER characterization; freeze the table; select classical operating points.", "Frozen baseline"),
        ("W5–W8", "Planned", "Train DJSCC and calibrate the task/reconstruction objective.", "Frozen learned model"),
        ("W9", "Planned", "Build and verify the task-aware digital feature control.", "Attribution control"),
        ("W10–W12", "Planned", "Validation rehearsal; freeze all choices; run one sealed test campaign.", "Hypothesis decisions"),
        ("W13–W17", "Planned", "Prepare demo, report, poster and final review package.", "Final submission"),
    ]
    for i, row in enumerate(rows):
        minimal_table_row(s, 2.06 + i * 0.80, widths, row, h=0.80,
                          sizes=(10.4, 10.0, 10.0, 10.0))
    s.text(0.78, 6.20, 11.45, 0.36,
           "First Review: 18–22 Aug · Second Review: 29 Sep–3 Oct · Final Review: 17–21 Nov · Report due: 20 Nov",
           size=12.4, font="sans", text_color=INK, align="center")
    s.text(0.78, 6.58, 11.45, 0.24,
           "W16 is report contingency, not room for new experimental scope.",
           size=11.3, font="sans", text_color=MUTED, align="center")
    minimal_footer(s); slides.append(s)

    # 11 — close on scope, not implementation trivia
    s = SlideScene(11, "Review decision", "What this First Review should confirm",
                   ("Objectives", "Subject Knowledge"),
                   "PR-3 standards register · PR-9 deployment dossier · acknowledgement PENDING")
    minimal_header(s, s.title)
    sections = [
        ("Research boundary", "Compare the three systems on image classification over normalized AWGN at matched complex-symbol budgets, using one sealed test campaign."),
        ("Technical claim boundary", "The conventional path uses OpenJPEG 2.5.4 and TS 38.212-derived LDPC/rate matching. The project does not claim a complete 5G NR radio link."),
        ("Deployment boundary", "Simulation-first Tier 1 is the required path. Conducted SDR replay and edge-device deployment remain optional stretch work after G-5."),
        ("Decision requested", "Confirm the task definition, the three-system comparison, the fairness controls and simulation-first Tier 1 as a sufficient capstone scope."),
    ]
    for i, (heading, body) in enumerate(sections):
        y = 1.46 + i * 1.12
        s.text(0.78, y, 2.72, 0.32, heading, size=14.8, bold=True,
               font="sans", text_color=INK)
        s.text(3.54, y - 0.02, 8.62, 0.64, body, size=13.2,
               font="sans", text_color=MUTED)
        s.line(0.78, y + 0.78, 11.40, 0, stroke=LINE, stroke_width=0.5)
    s.rect(0.78, 5.96, 11.40, 0.64, fill="F5F5F5", stroke=None, radius=0)
    s.text(1.02, 6.14, 10.92, 0.28,
           "Outstanding human item: record the guide's dated acknowledgement of the no-required-hardware path.",
           size=12.8, bold=True, font="sans", text_color=INK, align="center")
    minimal_footer(s); slides.append(s)

    # 12 — explicit rubric exposure and exact corrected H1 wording
    s = SlideScene(12, "Review map", "Formal H1 rule and First Review coverage",
                   ("Motivation", "Objectives", "Hypothesis", "Problem Survey", "Subject Knowledge", "Time Plan"),
                   "Exact protocol: SPEC §2 · full survey: docs/literature-review.md")
    minimal_header(s, s.title)
    s.text(0.78, 1.42, 6.72, 0.30, "H1 — low-SNR separation (confirmatory primary)",
           size=15.5, bold=True, font="sans", text_color=INK)
    s.text(0.78, 1.88, 6.76, 1.30,
           "Comparator: learned versus classical_adaptive at the headline ratio. A grid point qualifies when the studentized paired mean √N·Δ̂(s)/σ̂(s) exceeds 1.96. Let R_obs be the longest run of consecutive qualifying points at or below the training SNR. H1 is supported only if R_obs ≥ 3 and the calibrated run p-value ≤ 0.05.",
           size=12.6, font="sans", text_color=MUTED)
    s.text(0.78, 3.36, 6.76, 0.92,
           "The run rule decides H1. The mean paired accuracy difference over the full low-SNR region is the effect size of record. A curve crossing is reported if observed but is not required.",
           size=12.6, font="sans", text_color=MUTED)
    s.rect(0.78, 4.54, 6.76, 1.08, fill="F5F5F5", stroke=None, radius=0)
    s.text(1.02, 4.76, 6.28, 0.62,
           "Outcome rule: support, no support, or an adverse result are all valid project findings. The protocol does not change after test access.",
           size=12.8, bold=True, font="sans", text_color=INK)
    s.line(7.86, 1.42, 0, 4.92, stroke=LINE, stroke_width=0.7)
    s.text(8.18, 1.42, 4.00, 0.30, "Six-criterion rubric map", size=15.5,
           bold=True, font="sans", text_color=INK)
    rubric = [
        ("Motivation", "2–4"),
        ("Objectives", "1–2, 5, 8, 11"),
        ("Hypothesis", "8, 12"),
        ("Problem Survey", "6"),
        ("Subject Knowledge", "3–7, 9, 11–12"),
        ("Time Plan", "10"),
    ]
    for i, (criterion, slide_refs) in enumerate(rubric):
        y = 1.94 + i * 0.60
        s.text(8.18, y, 2.18, 0.28, criterion, size=12.2, bold=True,
               font="sans", text_color=INK)
        s.text(10.42, y, 1.70, 0.28, slide_refs, size=12.2,
               font="sans", text_color=MUTED, align="right")
        s.line(8.18, y + 0.38, 3.94, 0, stroke=LINE, stroke_width=0.4)
    s.text(8.18, 5.82, 3.94, 0.58,
           "Backing evidence: 30-source literature review, corrected Gantt, standards register, deployment dossier and authenticated gate records.",
