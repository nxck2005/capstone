...
def build_minimal_scenes() -> list[SlideScene]:
    ...
    s = SlideScene(5, "Problem survey", "What prior research establishes—and leaves open",
                   ("Problem Survey", "Subject Knowledge"),
                   "30-source synthesis: docs/literature-review.md")
    minimal_header(s, s.title)
    topics = [
    ...
    s = SlideScene(6, "Objectives", "Problem statement and project objectives",
                   ("Motivation", "Objectives"),
    ...
    s.text(3.26, 1.40, 8.88, 0.74,
           "Conventional image links protect reconstructed images and bits, while the receiver ultimately needs a correct class decision. A learned joint system may use the limited channel more effectively for that task, but the comparison must isolate task awareness from joint coding.",
           size=13.4, font="sans", text_color=MUTED)
    ...
    s.text(3.26, 2.57, 8.88, 0.78,
           "At the same bandwidth and channel conditions, can the learned joint system preserve classification accuracy better than a tuned conventional system, and is any remaining difference consistent with joint coding?",
    ...
    s.text(0.78, 3.80, 2.46, 0.30, "Objectives", size=15.5,
           bold=True, font="sans", text_color=INK)
    objectives = [
        "Build the conventional image, digital feature and learned joint systems.",
        "Match channel use, SNR, image identity and noise across all systems.",
        "Freeze every model and operating choice before opening the test split.",
        "Run one paired test campaign and report positive, null or adverse findings.",
    ]
    for i, item in enumerate(objectives, 1):
        y = 3.78 + (i - 1) * 0.64
    ...
    s = SlideScene(7, "Methodology", "Proposed methodology and comparison design",
                   ("Objectives", "Subject Knowledge"),
    ...
    s.text(1.00, 1.60, 10.98, 0.26,
           "Shared input for every arm: same image · same dataset split · same complex-symbol budget · same SNR · same keyed noise",
    ...
    rows = [
        ("Classical image", "JPEG 2000 → framing → LDPC → modulation → AWGN → image → classifier", "Strong conventional reference"),
        ("Digital feature", "Learned features → quantization → same digital physical layer → task head", "Separates task awareness from joint coding"),
        ("Learned joint", "Neural encoder → power normalization → AWGN → neural decoder and class head", "Tests end-to-end joint coding"),
    ]
    for i, row in enumerate(rows):
    ...
    s.text(2.98, 5.80, 9.18, 0.66,
           "Tune on validation only; freeze before test access; retain failed transmissions in accuracy; count all bytes and physical-layer identities; compare paired per-image outcomes.",
    ...
    s = SlideScene(8, "Hypothesis", "Hypotheses and evaluation plan",
                   ("Hypothesis", "Subject Knowledge"),
                   "Exact preregistered protocol: SPEC §2")
    minimal_header(s, s.title)
    s.text(0.78, 1.42, 6.90, 0.30, "H1 — low-SNR separation (confirmatory primary)",
           size=15.5, bold=True, font="sans", text_color=INK)
    s.text(0.78, 1.88, 6.92, 1.56,
           "Comparator: learned versus classical_adaptive at the headline ratio. A grid point qualifies when the studentized paired mean √N·Δ̂(s)/σ̂(s) exceeds 1.96. Let R_obs be the longest run of consecutive qualifying points at or below the training SNR. H1 is supported only if R_obs ≥ 3 and the calibrated run p-value ≤ 0.05.",
           size=12.5, font="sans", text_color=MUTED)
    s.text(0.78, 3.60, 6.92, 0.86,
    ...
    s = SlideScene(11, "Application and scope", "Application, scope and project risks",
                   ("Motivation", "Subject Knowledge", "Objectives"),
                   "Circular clause 5 · PR-3 standards register · PR-9 deployment dossier")
    minimal_header(s, s.title)
    columns = [
        (0.78, "Real-world application",
         "Remote cameras and edge sensors may need to support classification when bandwidth, latency or link reliability is limited. The receiver-owned task model represents split edge/cloud inference."),
        (4.42, "Required project scope",
         "Tier 1 is a simulation-based comparison of all three systems over normalized AWGN, with matched resources and one sealed test campaign. This is sufficient without radio hardware."),
    ...
    s.text(3.38, 5.47, 8.76, 0.58,
           "OpenJPEG 2.5.4 and TS 38.212-derived LDPC/rate matching over abstract AWGN. The project does not claim a complete 5G NR radio link.",
           size=12.7, font="sans", text_color=MUTED)
    ...
    # 12 — close with the proposal summary, decision request and rubric map
    s = SlideScene(12, "Review summary", "Review 1 summary and requested decisions",
    ...
    s.text(0.78, 1.42, 6.74, 0.30, "Project summary", size=15.5,
           bold=True, font="sans", text_color=INK)
    summary_rows = [
        ("Problem", "Remote classification must operate over a limited noisy link."),
        ("Proposal", "Train a task-aware joint encoder and decoder, then compare it with two matched digital systems."),
        ("Readiness", "The reference task, model feasibility, digital physical layer and conventional pipeline have passed their current gates."),
        ("Next work", "Finish characterization and selection, train the learned systems, freeze choices, then run the test campaign once."),
    ]
    ...
    s.text(1.02, 5.72, 2.02, 0.26, "Decision requested", size=13.0,
           bold=True, font="sans", text_color=INK)
    s.text(3.04, 5.67, 8.76, 0.58,
           "Confirm the research question, the three-system methodology, the fairness controls, the completion objectives and simulation-first Tier 1 as the project basis.",
