...
1. A camera must support classification across a limited noisy link.
2. The learned sender and receiver are the AIML method; image classification
   is the downstream evaluation task.
...
> An edge camera sees an image, but the server that must classify it is across
> a limited noisy link. We compare the normal way of sending the image with
> learned systems that decide what information is useful to send.
...
> An edge camera observes an image. The link has limited bandwidth and added
> noise. A server on the other side must decide what the image contains. The
> question is whether the camera must send every image detail for that decision.

Do not introduce architecture or loss terminology here. End by stating that
the project compares systems using the same channel budget.
...
After the plain explanation, name the method as DJSCC and supervised
end-to-end learning. Save the residual CNN, GroupNorm/PReLU, dual-head decoder
and `cross-entropy + λ × MSE` details for the methodology explanation.
