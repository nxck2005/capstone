...
import verify_g8_bler_characterization_manifest_v2 as source_verifier  # noqa: E402


MERGE_PATH = REPO_ROOT / "results/baseline/g8/bler_merge_report_v2.json"
TABLE_PATH = REPO_ROOT / "results/baseline/g8/bler_table_v2.json"
MERGE_RELATIVE_PATH = "results/baseline/g8/bler_merge_report_v2.json"
TABLE_RELATIVE_PATH = "results/baseline/g8/bler_table_v2.json"
MERGE_ID_PREFIX = "g8merge2"
...
def _historical(
    ...
    _require(request_attempts == sorted(set(request_attempts)), f"request history is not ordered: {work_unit_id}")
    _require(result_attempts == sorted(set(result_attempts)), f"result history is not ordered: {work_unit_id}")
    _require(set(result_attempts).issubset(set(request_attempts)), f"result lacks request: {work_unit_id}")
    _require(final_attempt in request_attempts and final_attempt in result_attempts, f"final attempt is not fully linked: {work_unit_id}")
    failures = 0
    request_only = 0
    for attempt in request_attempts:
        ...
        if attempt not in result_attempts:
            _require(attempt != final_attempt, f"final attempt is request-only: {work_unit_id}")
            request_only += 1
            continue
        ...
        if attempt == final_attempt:
            ...
            failures += 1
    return len(request_attempts), request_only, failures
...
def _expected_unit(
    ...
    attempts, request_only_attempts, failed_attempts = _historical(
        context,
        root,
        work_unit_id,
        final_attempt,
    ...
    source_epoch = 1 if ordinal <= 178 else 2
    return {
        ...
        "source_epoch_manifest_id": source_verifier.V1_MANIFEST_ID if source_epoch == 1 else source["manifest_id"],
...
def _verify_merge(
    ...
    expected_units = [_expected_unit(context, root, record, inspection["census"], source) for record in records]
    ...
    _require(merge["source_manifest_sha256"] == sha256_file(source_verifier.V2_MANIFEST), "merge source manifest SHA disagrees")
    _require(merge["source_epochs"] == source["source_epochs"], "merge source epoch chain disagrees")
...
def _table_from_merge(merge: Mapping[str, Any], source: Mapping[str, Any], merge_sha: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": 2,
        "artifact_role": "g8_bler_table_v2",
        "phase": "G8_C",
        "checkpoint": "C5",
        "table_id": None,
        ...
        "source_manifest_sha256": sha256_file(source_verifier.V2_MANIFEST),
...
def verify_coverage() -> dict[str, Any]:
    ...
    _require(merge["artifact_role"] == "g8_bler_merge_report_v2" and merge["phase"] == "G8_C", "merge artifact identity drift")
    ...
    merge_matches = [entry for entry in state["identity"]["produced_artifacts"] if entry["path"] == MERGE_RELATIVE_PATH]
    ...
    if merge_matches:
        _require(merge_matches == [{"path": MERGE_RELATIVE_PATH, "sha256": sha256_bytes(merge_raw), "bytes": len(merge_raw)}], "registered merge binding changed")
...
def verify(*, require_table_registered: bool = True) -> dict[str, Any]:
    ...
    _require(table["artifact_role"] == "g8_bler_table_v2" and table["phase"] == "G8_C", "table artifact identity drift")
    table_matches = [entry for entry in state["identity"]["produced_artifacts"] if entry["path"] == TABLE_RELATIVE_PATH]
    if require_table_registered:
        _require(len(table_matches) == 1, "BLER table is not registered exactly once")
        _require(table_matches == [{"path": TABLE_RELATIVE_PATH, "sha256": sha256_bytes(table_raw), "bytes": len(table_raw)}], "registered table binding changed")
    elif table_matches:
        _require(table_matches == [{"path": TABLE_RELATIVE_PATH, "sha256": sha256_bytes(table_raw), "bytes": len(table_raw)}], "registered table binding changed")
...
def main() -> int:
    ...
    try:
        if args.build:
            from baseline import g8_bler_characterization_v2 as characterization  # noqa: PLC0415
        ...
        if args.coverage_only:
            ...
            if args.register_merge:
                digest = _register_artifact(MERGE_RELATIVE_PATH, result["merge_raw"])
        ...
        if args.register_table:
            ...
            digest = _register_artifact(TABLE_RELATIVE_PATH, table_raw)
