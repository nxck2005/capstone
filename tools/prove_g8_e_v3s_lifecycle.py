#!/usr/bin/env python3
"""Prove the complete corrected-v3 worker-successor lifecycle with a tiny
merge-ineligible fixture (NON-SCIENTIFIC; never touches production runtime)."""
...
def fixture_context() -> dict[str, Any]:
    ...
    for index, (budget, modulation, rate) in enumerate(
        ...
        structural.append({
            "structural_identity_id": f"g8e-v3s-synthetic-structural-{chr(97 + index) * 64}",
            ...
            "ldpc_rate": rate,
            "encode_axis_px": 2,
            "packet_config_id": f"v3s-synthetic-packet-{index}",
            "payload_budget_bytes": budget,
            "packet_accounting": {"payload_bytes": budget},
        })
    authority = {
        "authority_id": "g8e-v3s-synthetic-authority-" + "b" * 64,
        "structural_identities": structural,
        "logical_candidate_to_structural_id": {},
    }
    data = {"data_identity_id": "g8e-v3s-synthetic-data-" + "8" * 64}
    direct = {"synthetic": True, "labels": list(LABELS)}
    classifier_sha = v3.sha256_bytes(b"synthetic-or-contract-bound-classifier")
    contract = {
        "campaign_id": "g8e-v3s-synthetic-campaign-" + "c" * 64,
        "contract_id": "g8e-v3s-synthetic-contract-" + "d" * 64,
        "execution_profile": {"profile_id": "synthetic-worker-profile", "device": "cuda:0"},
        "source_manifest": {"source_commit": "synthetic-v3s", "id": "synthetic-source", "sha256": "7" * 64},
        "authority": {"sha256": v3.sha256_bytes(v3.canonical_json(authority))},
        "scientific_data_identity": {"id": data["data_identity_id"], "path": "synthetic/data.json", "sha256": "9" * 64, "manifest_sha256": "6" * 64},
    ...
    for index, (sample_id, label) in enumerate(
        (("v3s-synthetic-delivered", 0), ("v3s-synthetic-infeasible", 2), ("v3s-synthetic-decode", 1))
    ):
    ...
    contract["scientific_data_identity"]["validation_count"] = len(sample_ids)
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=v3s.V3S_SYNTHETIC_PROOF_PATH)
    args = parser.parse_args()
    root = Path(tempfile.mkdtemp(prefix="g8e-v3s-lifecycle-"))
    try:
        ...
        try:
            ...
            if runner_cli.main(["--start", *common], fixture=partial_fixture) != 0:
                raise v3s.G8EV3SError("synthetic worker entry-point start failed")
            ...
            if runner_cli.main(["--resume", *common], fixture=resume_fixture) != 0:
                raise v3s.G8EV3SError("synthetic worker entry-point resume failed")
            ...
            if merge_cli.main(["--execute", "--runtime-root", str(runtime)], fixture=merge_fixture) != 0:
                raise v3s.G8EV3SError("synthetic worker E3 entry point failed")
            ...
            if aggregate_cli.main(
                ...
                raise v3s.G8EV3SError("synthetic worker E4 entry point failed")
            ...
            if not production_rejection:
                raise v3s.G8EV3SError("worker E3 accepted merge-ineligible fixture evidence")
            proof = {
                "schema_version": v3s.V3S_SCHEMA_VERSION,
                "artifact_role": "g8_e_v3s_non_scientific_full_lifecycle_proof",
                "status": "PASS",
                "record_labels": list(LABELS),
                ...
                "stages": {
                    ...
                    "actual_worker_runner_start": partial_state["completed_prefix_count"] == 3,
                    "interruption_after_partial_e2": True,
                    "actual_worker_runner_resume": True,
