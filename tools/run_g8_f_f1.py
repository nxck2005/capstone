...
F0 alone is intentionally insufficient for ``--start``.  This command requires
an additive owner-issued F1 launch authorization whose identity binds the F0
file and intended source commit.  F0 runs only ``--preflight`` and never calls
this command's production loop.
"""
    ...
    try:
        if args.preflight:
            ...
            print(json.dumps({
                ...
                "f1_started": False,
                "verdict": "F0 GREEN - F1 REQUIRES SEPARATE OWNER/OPERATOR LAUNCH",
            }, sort_keys=True))
            return 0
