...
def _worker_main(
    ...
    try:
        ...
        for work_unit_id in work_unit_ids:
            ...
            # ``result`` is the authenticated result-file wrapper.  The
            # measurement used for the worker summary is deliberately the
            # runner's top-level field; the old coordinator incorrectly
            # looked for result["measurement"].
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if maximum > 128:  # literal-ok: durable batch ceiling from the frozen G8_C protocol
            raise RuntimeError("--max-units-per-worker-batch may not exceed 128")
