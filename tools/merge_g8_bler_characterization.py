...
def _id(payload: Mapping[str, Any]) -> str:
    ...
    return f"g8merge-{sha256_bytes(canonical_json(body))}"
...
def _historical_attempts(
    ...
    _require(request_attempts == result_attempts, f"request/result attempt sets disagree for {work_unit_id}")
    ...
    for attempt in request_attempts:
        ...
        if attempt == final_attempt:
            ...
            failed += 1
    return len(request_attempts), failed
...
def _unit(
    ...
    attempts, failed_attempts = _historical_attempts(
        context,
        root,
        work_unit_id,
        final_attempt,
...
def build_report(root: Path) -> dict[str, Any]:
    ...
    units = [_unit(context, root, record, census) for record in classifications]
