#!/usr/bin/env python3
"""Publish corrected-v3 worker-successor E4 from one exact SHA-bound E3 closure."""
...
def main(argv: Sequence[str] | None = None, *, fixture: Mapping[str, Any] | None = None) -> int:
    ...
    try:
        ...
        if fixture is None:
            if args.runtime_root.resolve() != v3s.V3S_RUNTIME_ROOT.resolve() or e3_path.resolve() != v3s.V3S_E3_PATH.resolve():
                raise v3s.G8EV3SError("worker E4 must use the frozen v3s runtime/E3 paths")
            complete = v3s.verify_e3_complete(e3_path=e3_path, e3_sha256=args.e3_sha256)
            contract = complete["contract"]
            authority = v3s.load_measurement_authority()
            sample_ids = v3.verify_live_validation_identity(contract["scientific_data_identity"])
            production = True
        else:
