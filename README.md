...
**Current action: G8_B B3 — implement exact resume and merge validation.** On resumption, follow the single cursor in `instructions/RESUME.md`. **G8_B active. B0, B1, B1C, B2 and B2C complete. B3 next — implement exact resume and merge validation. No runner, simulation, smoke or characterization has started. G8_C–G8_G remain prohibited.**

**W4 and G8_A are complete. G8_B active. B0, B1, B1C, B2 and B2C complete. B3 next — implement exact resume and merge validation**, following `instructions/G8_B.txt` and the single cursor in `instructions/RESUME.md`. The full campaign was partitioned before data inspection into G8_A through G8_G under `instructions/`; G8_A froze the contract, policy bindings, 12,096 structural candidates, 3,213 required BLER work units and state primitives only. It started no characterization, validation measurement, sweep, training or adjudication. G8_B may build tooling and run bounded smoke only. B1 remains a historical design checkpoint; B1C corrected the frozen executable contract before data. Tooling, request and result schemas are now version 2, no version-1 request or result exists, and B2 and later must use the corrected contract only. B2 froze deterministic sharding, safe unit-state paths, closed snapshots, and atomic publication; it did not start a runner or interpret a state directory as execution history. B2C then corrected B2's publication mechanics before any data: first publication is crash-atomic (staged, fsynced, published with a descriptor-relative no-follow hard link that cannot replace), replacement is linearizable inside one exclusive per-unit lock so two writers with the same predecessor digest can never both succeed, symlink guards no longer follow the link, directory-fsync failures fail closed, a `result_linked` state is request-bound and terminal, every unit state binds the registered B2C state contract, and the independent verifier derives its own expected values rather than importing them from the module it checks. The characterization runner does not exist yet and **no runner, simulation, smoke or characterization has started. G8_C–G8_G remain prohibited.** The committed G-2 BLER evidence
...
records **sixteen amendment rounds** across 82 `AM` entries, and is the file to read before
...
```bash
.venv/bin/python tools/gen_spec_views.py --check       # 190 requirements, 10 generated files
