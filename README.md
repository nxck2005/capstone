...
**Current action: G8_B complete through B6; G8_C ready but not started.** On resumption, follow the single cursor in `instructions/RESUME.md`. The live state is `G8_B/tooling_smoke_complete` with seven artifacts, empty/null work-unit IDs and zero counters. No full-strength characterization, BLER table, selection, authorization, inference, training, validation decoding or test access has occurred. The exact G8_C restart command is `rg -n "G8_C|characterization_open|full_strength|run_g8_bler|resume_plan|merge_report|tooling_smoke_complete" src/baseline tools tests instructions`.

**W4 and G8_A are complete. G8_B is complete through B6**, following `instructions/G8_B.txt`, `instructions/G8_B_REMAINDER.txt` and the single cursor in `instructions/RESUME.md`. G8_A froze the contract, policy bindings, 12,096 structural candidates, 3,213 required BLER work units and state primitives before data. G8_B built and independently verified the authenticated runner, exact resume/merge machinery, crash-atomic publication and bounded smoke. The registered v3 runner and schema-2 smoke bindings are live; the bounded smoke is non-scientific, nonmergeable and zero-coverage. **No full-strength characterization, BLER table, selection, authorization, validation measurement, inference, training or test access has started. G8_C is ready but not started.** The committed G-2 BLER evidence
